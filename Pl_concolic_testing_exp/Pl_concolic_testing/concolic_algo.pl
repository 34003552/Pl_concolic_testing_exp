...
:- module(concolic_algo, [mainT/4, testcases/1, pending_test_case/1]).

:- include('extras/headers/h_concolic_algo.pl').
...
eval(CGoal, [A|RA], [B|RB], Trace, SGoal, Gamma, G) :-
    ...
    traces(Traces), (
        member__list_any(Trace, Traces) -> true ;
    ...
    append__unk(Gamma_, NewGamma_, NewGamma),
...
get_new_trace(Trace, Alts, Traces, NewTrace) :-
    ...
    member__list_any(Labels, LPower),  %% nondeterministic!!
    append__T0(PTrace, [Labels], NewTrace),
...
alts(SGoal, Gamma, Atom, Labels, AllLabels, G, NewGoals) :-
    ...
    findall(NewGoal, (
        member__list_any(LPos, LPower), LPos \== Labels,
        subtract(AllLabels, LPos, LNeg),
        get_constraints(Atom, G, LPos, LNeg, NewConstr),
        append__unk(Gamma, NewConstr, Constr),
...
get_constraints(A, G, LPos, LNeg, Constrs) :-
    ...
    append__unk(PosConsts, NegConsts, Constrs).
