...
:- module(pl_newtyper, [proceed/1, types/2, preds/1, pl_newtyper_to_std_types/1, pl_newtyper_to_std_predtypes/1]).
...
list_file_preds(FilePath, Preds) :-
    absolute_file_name(FilePath, FilePath_),
    (source_file(FilePath_) ->
        source_file_property(FilePath_, load_context(Mod, _, _)),
        findall(PName/PArity, (current_predicate(_, Mod:Pred), source_file(Mod:Pred, FilePath_), functor(Pred, PName, PArity)), Preds)
    ;
        load_files(FilePath), list_file_preds(FilePath, Preds), unload_file(FilePath)
    ).
...
match_wlinks :- findall(WLink, w_link(WLink), WLinks), %format("Weak links before: ~w~n", [WLinks]),
    match_wlinks(WLinks),
    findall(WLink_, w_link(WLink_), _WLinks_). %format("Weak links after: ~w~n", [WLinks_]).
...
proceed(FilePath) :-
    ...
    match_wlinks,
    mk_types, writeln("done!"), mk_preds.
    %findall(TC, type_clusters(TC), TCs), print(TCs), nl.



pl_newtyper_to_std_types(StdTypes) :- 
    findall(Type_-Enum, (pl_newtyper:types(Type, Enum), atom_string(Type_, Type)), Types),
    append([['$built-ins$'], Types], StdTypes).

pl_newtyper_to_std_predtypes(StdPredTypes) :- 
    findall(Pred, pl_newtyper:preds(Pred), StdPredTypes).