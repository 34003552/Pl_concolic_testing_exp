...
Z3_symbol z3Tools::get_accessor_name(Z3_context ctx, const char *name)
{
    Z3_symbol is_sym;
    char *buffer = (char*)malloc (4 + strlen(name));
    strcpy(buffer, "is_");
    strcat(buffer, name);
    is_sym = Z3_mk_string_symbol(ctx, buffer);
    free(buffer);

    return is_sym;
}
Z3_symbol z3Tools::get_node_accessor_name(Z3_context ctx, const char *name, int j)
{
    Z3_symbol arg;
    char *buffer = (char*)malloc (10 + strlen(name));
    strcpy(buffer, name);
    strcat(buffer, "_arg_");
    strcat(buffer, itoa(j,10));
    arg = Z3_mk_string_symbol(ctx, buffer);
    free (buffer);
