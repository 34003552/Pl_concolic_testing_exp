...
char *atom__to_chars(atom *this) {
    size_t len = 1 + strlen(this->name) + 1 + 1;
    char *str = malloc(len);
...
char *Functor__to_chars(Functor *this) {
    size_t len = 1 + strlen(this->name) + 1 + 1 + 2 + 1; // null-terminated
    char *str = malloc(len);
...
void String__drain(String *this) {
    free((void*)*this);
}
void Functor__drain(Functor *this) {
    free((void*)this->name);
}
void Term__drain(Term *this) {
    Functor__drain(&this->functor);
    TermList tl = this->args;
    TermListIterator tli = TermList__begin(&tl);
    while (tli.m_current != NULL) {
        Term__drain(tli.m_current->m_elem);
        tli.m_current = tli.m_current->m_next;
    }
}
void Datatype__drain(Datatype *this) {
    String__drain(&this->name);
    Datatype__ConstructorList ctl = this->ctors;
    Datatype__ConstructorListIterator ctli = Datatype__ConstructorList__begin(&ctl);
    while (ctli.m_current != NULL) {
        Datatype__Constructor *ct = ctli.m_current->m_elem;
        String__drain(&ct->name);
        Datatype__Constructor__AccessorList acl = ct->accs;
        Datatype__Constructor__AccessorListIterator acli = Datatype__Constructor__AccessorList__begin(&acl);
        while (acli.m_current != NULL) {
            Datatype__Constructor__Accessor *ac = acli.m_current->m_elem;
            String__drain(&ac->name);
            String__drain(&ac->type);
            free((void*)ac);
            acli.m_current = acli.m_current->m_next;
        }
        free((void*)ct);
        ctli.m_current = ctli.m_current->m_next;
    }
}