...
void z3Manager::z3Bundle::mk_term_type(TList *tlist, bool need_int, bool need_lists)
{
    ...
    if (need_int)
    {
        ...
        Z3_func_decl int_ctor, int_tst, int_accs[1];
        Z3_query_constructor(m_ctx, cons__term[id__term_from_int], 1, &int_ctor, &int_tst, int_accs);
        m_types["Term"].ctors.push_back({ // term_from_int : Int -> Term
            consnames__term[id__term_from_int], // term_from_int
            int_ctor, // Z3_mk_func_decl(m_ctx, consnames__term[id__term_from_int], 1, &int_sort, sorts[ID_TERM]),
            {}
        });
        m_types["Term"].ctors.back().accs.push_back({ // term_as_int : Term -> Int
            accnames__term_from_int[0], // term_as_int
            int_accs[0] // Z3_mk_func_decl(m_ctx, accnames__term_from_int[0], 1, &sorts[ID_TERM], int_sort)
        });
    }

    if (need_lists)
    {
        Z3_func_decl nil_ctor, nil_tst;
        Z3_query_constructor(m_ctx, cons__termlist[id__nil], 0, &nil_ctor, &nil_tst, 0);
        m_types["TermList"].ctors.push_back({ // nil : TermList
            consnames__termlist[id__nil], // nil
            nil_ctor, // Z3_mk_func_decl(m_ctx, consnames__termlist[id__nil], 0, 0, sorts[ID_TERMLIST]),
            {}
        });

        Z3_func_decl insert_ctor, insert_tst, insert_accs[2];
        Z3_query_constructor(m_ctx, cons__termlist[id__insert], 2, &insert_ctor, &insert_tst, insert_accs);
        m_types["TermList"].ctors.push_back({ // insert : (Term, TermList) -> TermList
            consnames__termlist[id__insert], // insert
            insert_ctor, // Z3_mk_func_decl(m_ctx, consnames__termlist[id__insert], 2, sorts, sorts[ID_TERMLIST]),
            {}
        });
        m_types["TermList"].ctors.back().accs.push_back({ // head : TermList -> Term
            accnames__insert[0], // head
            insert_accs[0] // Z3_mk_func_decl(m_ctx, accnames__insert[0], 1, &sorts[ID_TERMLIST], sorts[ID_TERM])
        });
        m_types["TermList"].ctors.back().accs.push_back({ // tail : TermList -> TermList
            accnames__insert[1], // tail
            insert_accs[1] // Z3_mk_func_decl(m_ctx, accnames__insert[1], 1, &sorts[ID_TERMLIST], sorts[ID_TERMLIST])
        });


        Z3_func_decl cons_ctor, cons_tst, cons_accs[1];
        Z3_query_constructor(m_ctx, cons__term[id__cons], 1, &cons_ctor, &cons_tst, cons_accs);
        m_types["Term"].ctors.push_back({ // cons : TermList -> Term
            consnames__term[id__cons],
            cons_ctor, // Z3_mk_func_decl(m_ctx, consnames__term[id__cons], 1, &sorts[ID_TERMLIST], sorts[ID_TERM]),
            {}
        });
        m_types["Term"].ctors.back().accs.push_back({ // list : Term -> TermList
            accnames__cons[0], // list
            cons_accs[0], // Z3_mk_func_decl(m_ctx, accnames__cons[0], 1, &sorts[ID_TERM], sorts[ID_TERMLIST])
        });
    }
}
